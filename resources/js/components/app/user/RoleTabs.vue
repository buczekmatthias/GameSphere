<script setup lang="ts">
import { ZiggyWithGamesQuery } from '@/pages/app/Users.vue';
import { Link } from '@inertiajs/vue3';
import { capitalize, computed } from 'vue';

const props = defineProps<{
    roles: string[];
    ziggy: ZiggyWithGamesQuery;
    total: number;
    reloadOnly: string[];
}>();

const queryWithoutRole = computed(() => (props.ziggy.query.contains ? { contains: props.ziggy.query.contains } : {}));
</script>

<template>
    <div class="flex gap-2.5 overflow-auto py-1.5 text-sm text-nowrap">
        <Link
            :href="route(ziggy.current, queryWithoutRole)"
            class="rounded-md px-2 py-1.5"
            :class="{ 'bg-primary text-background pointer-events-none': !ziggy.query.role, 'hover:bg-primary/20 duration-150': ziggy.query.role }"
            :only="reloadOnly"
            as="button"
        >
            All
        </Link>
        <Link
            :href="route(ziggy.current, { ...ziggy.query, role: role })"
            class="rounded-md px-2 py-1.5"
            v-for="role in roles"
            :key="role"
            :class="{
                'bg-primary text-background pointer-events-none': ziggy.query.role === role,
                'hover:bg-primary/30 duration-150': ziggy.query.role !== role,
            }"
            :only="reloadOnly"
            as="button"
        >
            {{ capitalize(role.replace('_', ' ')) }}s
        </Link>
    </div>
</template>
